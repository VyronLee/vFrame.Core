...
namespace vFrame.Core
{
    ...
    {
        private readonly Dictionary<MemberInfo, IValueComparer> _overridesByMember =
            new Dictionary<MemberInfo, IValueComparer>();

        private readonly List<Tuple<Func<MemberInfo, bool>, IValueComparer>> _overridesByMemberFilter =
            new List<Tuple<Func<MemberInfo, bool>, IValueComparer>>();

        private readonly Dictionary<string, List<ValueComparerWithFilter>> _overridesByName =
            new Dictionary<string, List<ValueComparerWithFilter>>();

        private readonly Dictionary<Type, List<ValueComparerWithFilter>> _overridesByType =
            new Dictionary<Type, List<ValueComparerWithFilter>>();
        ...
        public void AddComparer(Type type, IValueComparer valueComparer,
            ...
            if (!_overridesByType[type]
                    .Any(comparer => comparer.ValueComparer == valueComparer && comparer.Filter == filter)) {
