...
namespace vFrame.Core
{
    ...
    public sealed class LoggerCategory : ILogger
    {
        ...
        public bool IsEnabled(LogLevelDef level) =>
            MinimumLevel <= level && Logger.IsEnabled(level);

        public void Trace(string text) {
            if (!IsEnabled(LogLevelDef.Trace)) return;
            Logger.Trace(_tag, text);
        }

        public void Debug(string text) {
            if (!IsEnabled(LogLevelDef.Debug)) return;
            Logger.Debug(_tag, text);
        }

        public void Info(string text) {
            if (!IsEnabled(LogLevelDef.Info)) return;
            Logger.Info(_tag, text);
        }

        public void Warning(string text) {
            if (!IsEnabled(LogLevelDef.Warning)) return;
            Logger.Warning(_tag, text);
        }

        public void Error(string text) {
            if (!IsEnabled(LogLevelDef.Error)) return;
            Logger.Error(_tag, text);
        }

        public void Fatal(string text) {
            if (!IsEnabled(LogLevelDef.Fatal)) return;
