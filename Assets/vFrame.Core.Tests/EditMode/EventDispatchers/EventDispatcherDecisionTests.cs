...
namespace vFrame.Core.Tests.EditMode.EventDispatchers
{
    public class EventDispatcherDecisionTests
    {
        ...
        [Test]
        public void Listen_ReturnsDecisionSubscription() {
            var dispatcher = CreateDispatcher();

            var subscription = dispatcher.Listen<TestDecision>(_ => true);

            Assert.That(subscription, Is.Not.Null);
            Assert.That(subscription, Is.InstanceOf<IDecisionSubscription>());
            Assert.That(subscription.Handle, Is.GreaterThan(0));
        }

        [Test]
        public void Unsubscribe_RemovesDecisionSubscription() {
            var dispatcher = CreateDispatcher();
            var count = 0;
        ...
        [Test]
        public void Decide_RemainsDistinctFromMessageDispatch() {
            var dispatcher = CreateDispatcher();
            var decisionCount = 0;
            var messageCount = 0;
            ...
            dispatcher.Subscribe<TestMessage>(_ => messageCount++);
